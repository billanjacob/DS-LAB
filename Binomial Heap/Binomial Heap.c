...
BIN_HEAP *make_BIN_heap();
void insertion(BIN_HEAP *H, NODE *new, int val);
NODE *extract_min(BIN_HEAP *H);
void consolidate(BIN_HEAP *H);
void BIN_heap_link(BIN_HEAP *H, NODE *y, NODE *x);
NODE *find_min_node(BIN_HEAP *H);
void decrease_key(BIN_HEAP *H, NODE *node, int key);
void cut(BIN_HEAP *H, NODE *node_to_be_decrease, NODE *parent_node);
void cascading_cut(BIN_HEAP *H, NODE *parent_node);
void Delete_Node(BIN_HEAP *H, int dec_key);
BIN_HEAP *make_BIN_heap()
{
BIN_HEAP *H;
...
if (H == NULL)
{
printf(" \n Binomial heap not yet created \n");
return NULL;
}
else
return H->min;
}
BIN_HEAP *unionHeap(BIN_HEAP *H1, BIN_HEAP *H2)
{
BIN_HEAP *Hnew;
Hnew = make_BIN_heap();
Hnew->min = H1->min;
...
void decrease_key(BIN_HEAP *H, NODE *node_to_be_decrease, int new_key)
{
NODE *parent_node;
if (H == NULL)
{
printf("\n Binomial heap not created ");
return;
}
...
void *find_node(BIN_HEAP *H, NODE *n, int key, int new_key)
{
NODE *find_use = n;
NODE *f = NULL;
...
BIN_HEAP *insertion_procedure()
{
BIN_HEAP *temp;
int no_of_nodes, ele, i;
NODE *new_node;
temp = (BIN_HEAP *)malloc(sizeof(BIN_HEAP));
temp = NULL;
if (temp == NULL)
{
temp = make_BIN_heap();
...
if (heap == NULL)
{
printf("\n no Binomial heap is created please create Binomial heap \n ");
break;
}
